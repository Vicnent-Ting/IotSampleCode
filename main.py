...
SOM_MAP = {'24':'tx2','33':'nano','25':'nx'}
## for DPS Testing
model_id = "dtmi:AIMobile:M1_H3;1"
# For Multiple components
import pnp_helper
windows_device_info_component_name = "WindowsDeviceInfo1"
linux_device_info_component_name = "deviceinfo"
linux_device_telemetry_component_name = 'thermalzones'
#================#
global period
period = 2
...
def getCPUID():
    proc = subprocess.Popen('cat /sys/module/tegra_fuse/parameters/tegra_chip_id'.split(),stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    out,err = proc.communicate()
    cpuid = out.decode().strip('\n')
    return cpuid

def getGpuTemp():
    try:
        p = subprocess.Popen('cat /sys/class/thermal/thermal_zone2/temp'.split(),stdout=subprocess.PIPE)
        out,err = p.communicate()
        return int(out.decode().split()[0])/1000
...
def getGpuUsage():
    try:
        proc = subprocess.Popen('sudo /home/ivs/tegrastats | head -n 1',shell=True,stdout=subprocess.PIPE)
        out,err =proc.communicate()
        out = out.decode()
        index1 = out.index('GR3D_FREQ')
        out = out[index1:]
        index2 = out.find(' ')
        index3 = out.find('%')
        return int(out[index2+1:index3])
    except Exception as e:
        print(str(e))
        return 0
...
async def property_update(device_client,os_type,machine):
    ...
    gpuName = SOM_MAP.get(getCPUID(),'nvidia')
    ...
    if os_type == "Windows":
        ...
        properties_device_info = pnp_helper.create_reported_properties(
            ...
            CPUName=cpuInfo,
            CPUCore=str(cpuCores),
            RAMTotalSize=str(memTotal),
            GPUName=gpuName,
            GPUNumber='1',
        )
    print('send linux info')       
    print(properties_device_info) 
    ...
    while True:
        if os_type == "Linux":
            if 'x86' in machine:
                ...
                currentTemp = psutil.sensors_temperatures()['MCPU-therm'][0][1]
                #currentTempGPU = psutil.sensors_temperatures()['gpu-thermal'][0][1]
        
        json_msg = {}
        json_msg["CPUThermal"]=currentTemp
        json_msg["CPUUsage"]=psutil.cpu_percent()
        json_msg["GPUThermal"]=getGpuTemp()
        json_msg["GPUUsage"]=getGpuUsage()
        json_msg["RAMAvailable"]=psutil.virtual_memory().free
        ...
        if os_type == "Windows":
            ...
            await send_telemetry_with_component_name(device_client, json_msg, linux_device_telemetry_component_name)
